...
export class GitHubProjectV2SyncProxyImpl implements IGitHubProjectV2SyncProxy {
  async syncGitHubProjectV2(): Promise<void> {
    return await window.electron.ipcRenderer.invoke(IpcChannel.GITHUB_PROJECT_V2_SYNC_PROJECT);
  }
  ...
  async syncGitHubProjectV2Item(projectId: string): Promise<void> {
    return await window.electron.ipcRenderer.invoke(IpcChannel.GITHUB_TASK_SYNC, projectId);
  }
}
